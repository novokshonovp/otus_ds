...
from storages.sb_storage import SbStorage
from parsers.sb_parser import SbParser
from stats.sb_stat import SbStat
...
def gather_process():
    logger.info("gather")
    storage = SbStorage(SCRAPPED_FILE)
...
def convert_data_to_table_format():
    ...
    parser = SbParser(['name', 'href', 'advertise_date',
                       'price', 'location', 'hull', 'year', 'length'])
    df = parser.parse(storage)
    df.to_csv(TABLE_FORMAT_FILE, sep=SEPARATOR, encoding='utf-8')
...
def stats_of_data():
    ...
    # Ask yourself what would you like to know about this data (most frequent
    # word, or something else)
    df = pd.read_csv(TABLE_FORMAT_FILE, sep=SEPARATOR)
    stat_builder = SbStat(df)

    decade_starting_date, decade_ending_date, max_number_of_advs = stat_builder.biggest_amount_of_advertisement_by_decade()
    print(
        f'Biggest number of advertisements ({max_number_of_advs}) is for boats were building between {decade_starting_date} - {decade_ending_date} years.')

    price_mean = stat_builder.average_price(
        decade_starting_date=decade_starting_date, length=range(31, 41))
    print(
        f'Average price for 31-41 foots boats in decade ({decade_starting_date} - {decade_ending_date}) is ${price_mean}')
